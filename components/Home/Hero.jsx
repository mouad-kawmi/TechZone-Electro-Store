          ...
          <div className="hero-content space-y-10 lg:pr-10">
            ...
            <div className="flex flex-wrap items-center gap-6 pt-4">
              <button
                ...
                className="group bg-slate-900 dark:bg-white text-white dark:text-slate-950 px-10 py-5 rounded-2xl font-bold text-xs uppercase tracking-widest transition-all hover:scale-105 active:scale-95 flex items-center gap-3"
              >
                Explorer le Store
              ...
              <div className="flex -space-x-4">
                {[
                  { init: 'MA', bg: 'bg-blue-600' },
                  { init: 'SK', bg: 'bg-indigo-600' },
                  { init: 'YL', bg: 'bg-slate-900' },
                  { init: 'RH', bg: 'bg-emerald-600' }
                ].map((u, i) => (
                  <div key={i} className={`size-10 rounded-full border-2 border-white dark:border-slate-950 ${u.bg} flex items-center justify-center shadow-lg`}>
                    <span className="text-[10px] font-black text-white">{u.init}</span>
                  </div>
                ))}
