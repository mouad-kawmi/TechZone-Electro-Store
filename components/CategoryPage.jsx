...
const CategoryPage = ({
  ...
  const filtered = useMemo(() => {
    return products.filter(p => {
      ...
      const matchRam = selectedRam.length === 0 || (p.technicalSpecs?.ram && selectedRam.includes(p.technicalSpecs.ram));
      const matchStorage = selectedStorage.length === 0 || (p.technicalSpecs?.storage && selectedStorage.includes(p.technicalSpecs.storage));
