...
const ProductDetails = ({
  ...
  const initialStorage = product.variations?.storage?.[0];
  const [selectedStorage, setSelectedStorage] = useState(typeof initialStorage === 'object' ? initialStorage.name : initialStorage);
  ...
  const availableStock = useMemo(() => {
    ...
    if (product.variations?.storage) {
      const storageVar = product.variations.storage.find(s =>
        (typeof s === 'object' ? s.name : s) === selectedStorage
      );
      return typeof storageVar === 'object' ? storageVar.stock : product.stock;
    }
                    ...
                    <div className="flex flex-wrap gap-2">
                      <div className="flex flex-wrap gap-3">
                        {product.variations.storage.map((item, idx) => {
                          ...
                          const stock = typeof item === 'object' ? item.stock : 10;
                          const isAvailable = stock > 0;

                          return (
                            <button
                              key={idx}
                              onClick={() => isAvailable && setSelectedStorage(name)}
                              className={`group relative px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${selectedStorage === name
                            ...
                            >
                              <span className={!isAvailable ? 'line-through decoration-slate-400' : ''}>{name}</span>
                              {!isAvailable && (
                                <span className="absolute -top-2 -right-2 bg-rose-500 text-white text-[7px] px-1.5 py-0.5 rounded-full shadow-lg font-black scale-90">Rupture</span>
                              )}
                            </button>
                          );
                        })}
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 sm:gap-6">
                    <div className="flex items-center justify-center bg-slate-50 dark:bg-slate-800 rounded-2xl p-2 border border-slate-100 dark:border-slate-700">
                      ...
                      <button
                        ...
                        className={`size-10 flex items-center justify-center transition-colors ${quantity >= availableStock ? 'text-slate-300 cursor-not-allowed' : 'text-slate-500 hover:text-blue-600'}`}
                      >
